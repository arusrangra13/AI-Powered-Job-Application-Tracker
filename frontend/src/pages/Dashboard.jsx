  ...
  return (
    <div className="stat-card glass card-hover" style={{ border: `1px solid ${color}25` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
        ...
        <span style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--text-color)' }}>{value}</span>
      </div>
      <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</div>
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--text-color)', marginBottom: '0.25rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
        <StatCard label="Total Applied" value={s.total || 0} icon={Briefcase} color="var(--accent-strong)" />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <div className="glass" style={{ borderRadius: '1rem', padding: '1.5rem', border: '1px solid rgba(100,120,249,0.12)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
            <h2 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-color)' }}>Response Rate</h2>
            <TrendingUp size={18} color="var(--accent-strong)" />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
            <div className="score-ring">
              ...
              <div style={{ position: 'absolute', textAlign: 'center' }}>
                <div style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-color)' }}>{s.responseRate || 0}%</div>
              </div>
            </div>
            <div>
              <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>Status breakdown</div>
              {[['Applied', s.applied, '#60a5fa'], ['Interview', s.interview, '#fbbf24'], ['Offer', s.offer, '#4ade80'], ['Rejected', s.rejected, '#f87171']].map(([l, v, c]) => (
                <div key={l} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', color: '#94a3b8' }}>{l}: <strong style={{ color: 'var(--text-color)' }}>{v || 0}</strong></span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="glass" style={{ borderRadius: '1rem', padding: '1.5rem', border: '1px solid rgba(100,120,249,0.12)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
            <h2 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-color)' }}>Upcoming Reminders</h2>
            <Link to="/reminders" style={{ color: 'var(--accent-strong)', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.25rem', textDecoration: 'none' }}>
              View all <ArrowRight size={12} />
            </Link>
          </div>
          {reminders.length === 0 ? (
            <div style={{ color: 'var(--text-muted)', fontSize: '0.875rem', textAlign: 'center', padding: '1.5rem 0' }}>
              <Bell size={32} style={{ color: 'var(--surface-border)', marginBottom: '0.5rem' }} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {reminders.map(r => (
                <div key={r.id} className="glass-light" style={{ borderRadius: '0.75rem', padding: '0.75rem' }}>
                  <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-color)' }}>{r.title}</div>
                  {r.application?.job && (
                    <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{r.application.job.company}</div>
                  )}
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', marginTop: '0.25rem' }}>
                    <Calendar size={11} color="var(--accent-strong)" />
                    <span style={{ fontSize: '0.7rem', color: 'var(--accent-strong)' }}>{format(parseISO(r.dueDate), 'MMM d, yyyy')}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="glass" style={{ borderRadius: '1rem', padding: '1.5rem', border: '1px solid rgba(100,120,249,0.12)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
          <h2 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-color)' }}>Recent Applications</h2>
          <Link to="/kanban" style={{ color: 'var(--accent-strong)', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.25rem', textDecoration: 'none' }}>
            View Kanban <ArrowRight size={12} />
          </Link>
        </div>
        {recent.length === 0 ? (
          <div style={{ color: 'var(--text-muted)', fontSize: '0.875rem', textAlign: 'center', padding: '2rem' }}>
            No applications yet. <Link to="/jobs" style={{ color: 'var(--accent-strong)' }}>Add your first job →</Link>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {recent.map(app => (
              <div key={app.id} className="glass-light card-hover" style={{ borderRadius: '0.75rem', padding: '0.875rem 1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontWeight: 600, fontSize: '0.875rem', color: 'var(--text-color)' }}>{app.job.title}</div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{app.job.company} {app.job.location ? `• ${app.job.location}` : ''}</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{format(parseISO(app.appliedDate), 'MMM d')}</span>
